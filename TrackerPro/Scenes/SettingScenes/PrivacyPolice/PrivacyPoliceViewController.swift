...
final class PrivacyPoliceViewController: UIViewController {
    
    private let backView: CustomButton = {
        let vw = CustomButton()
        vw.image = ImageHelper.backOtherIcon.withTintColor(.systemGreen)
    ...
    private let titleLabel: UILabel = {
        ...
        lb.text = "Конфиденциальность"
        return lb
    }()
    
    private let viewModel: PrivacyPoliceViewModel
    
    init(viewModel: PrivacyPoliceViewModel) {
